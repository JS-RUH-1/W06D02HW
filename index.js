...
app.put('/games', (req, res) => {
    if(!req.body.id) return res.json({error:"id must be entered"})
    fs.readFile('games.json', {encoding: 'utf8'}, (err, fileData) => {
        let data = JSON.parse(fileData);
        let index = data.findIndex(game => game.id === req.body.id);
        data[index] = req.body;
...
app.delete('/gamesDelete/:id', (req, res) => {
    if(!req.params.id) return res.status(502).json({error: "param is required"});
    ...
    fs.readFile('games.json', {encoding: 'utf8'}, (err, fileData) => {
        ...
        fs.writeFile('games.json', JSON.stringify(data.filter(game => game.id !== parseInt(req.params.id))), (err) => {
            res.json(toDelete);
        })
